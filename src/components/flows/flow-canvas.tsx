...
function CanvasAddNodeButton() {
  ...
  return (
    <div className="relative" ref={containerRef}>
      ...
      {open && (
        <div className="absolute left-0 mt-1.5 z-50 w-[268px] rounded-lg bg-popover p-1.5 text-popover-foreground shadow-lg ring-1 ring-foreground/10 border border-border flex flex-col gap-1 max-h-96 overflow-y-auto">
          {groupNodeTypesByCategory(ADD_NODE_TYPES).map((group, i) => (
            <div key={group.id} className="flex flex-col">
              {i > 0 && <div className="my-1 border-t border-border" />}
              <div className="px-2 py-1 text-muted-foreground text-[10px] font-semibold tracking-wider uppercase select-none">
                {group.label}
              </div>
              {group.types.map((t) => {
                ...
                return (
                  <button
                    ...
                    className="w-full text-left flex items-center gap-3 rounded-md px-2 py-2 text-sm text-foreground transition-colors hover:bg-accent hover:text-accent-foreground"
                  >
